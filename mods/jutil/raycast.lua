...
--]]
function jutil.check_point_box(hit, B1, B2, Axis)
	if Axis == 1 then
...
function jutil.check_line_box(B1, B2, L1, L2)
	...
	for axis, name in pairs({'x', 'y', 'z'}) do
		...
		if did_hit_a and jutil.check_point_box(ret_a, B1, B2, axis) then
			return true, ret_a, name .. '-';
		end

		local did_hit_b, ret_b = get_intersection(
				L1[name]-B2[name], L2[name]-B2[name], L1, L2)
		if did_hit_b and jutil.check_point_box(ret_b, B1, B2, axis) then
...
--]]
function jutil.get_node_boxes(def)
...
function jutil.raytrace_blocks(a, b, step)
	...
	for pos, apos in jutil.block_iter(a, b, step or 0.4) do
		...
		local node_cols = jutil.get_node_boxes(node_def);
